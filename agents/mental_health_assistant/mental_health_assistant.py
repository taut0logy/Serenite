...
cassio.init(
    token=os.getenv("ASTRA_DB_APPLICATION_TOKEN"), database_id=os.getenv("ASTRA_DB_ID")
)
...
class EmotionAnalysis(BaseModel):
    ...
    primary_emotion: str = Field(
        description="The primary emotion detected in the message"
    )
    emotion_justification: str = Field(
        description="Justification for why this emotion was detected"
    )
    crisis_level: str = Field(
        description="Assessed crisis level: low, medium, high, or very_high"
    )
    crisis_justification: str = Field(
        description="Justification for the crisis level assessment"
    )
    needs_immediate_resources: bool = Field(
        description="Whether immediate crisis resources are needed"
    )
    reasoning: str = Field(
        description="Step-by-step reasoning about the emotional analysis"
    )
...
class ResponseStrategy(BaseModel):
    ...
    approach: str = Field(
        description="Emotional approach to take (empathize, validate, encourage, educate, etc)"
    )
    key_points: List[str] = Field(description="Key points to address in the response")
    appropriate_tools: List[str] = Field(
        description="Tools that might be helpful (if any)"
    )
    reasoning: str = Field(
        description="Step-by-step reasoning for this response strategy"
    )
...
def get_youtube_transcript_and_summary(url: str) -> Dict[str, str]:
    ...
    try:
        ...
        summary_prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessage(
                    content="You are a mental health expert who creates helpful summaries of videos."
                ),
                HumanMessage(
                    content=f"Create a helpful summary of this video transcript in the context of mental health support. Include key points and advice.\n\nTRANSCRIPT:\n{transcript}"
                ),
            ]
        )
...
def generate_video_blog(url: str) -> Dict[str, str]:
    ...
    try:
        ...
        blog_prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessage(
                    content="""You are a professional mental health content writer who creates 
            ...
            Maximum length: 600 words."""
                ...
                HumanMessage(
                    content=f"""Create a blog post based on this mental health video transcript.
        ...
        key_points = []
        if (
            "key points" in content.lower()
            or "highlights" in content.lower()
            or "takeaway" in content.lower()
        ):
        ...
        if not key_points:
            key_points_prompt = ChatPromptTemplate.from_messages(
                [
                    SystemMessage(
                        content="Extract 3-5 key points from this blog post as bullet points:"
                    ),
        ...
        return {"title": video_title, "content": content, "key_points": key_points}
    except Exception as e:
        return {
...
def suggest_self_care(state: AgentState) -> Dict:
    ...
    try:
        ...
        selected_activity = random.choice(
            primary_activities
        )  # Bias toward emotion-specific activities
        alternative_activity = random.choice(
            [a for a in all_activities if a != selected_activity]
        )

        activity_benefit = ""
        if emotion.lower() in ["anxious", "stressed", "fearful", "worried"]:
            activity_benefit = (
                "ground you in the present moment and calm your nervous system"
            )
        elif emotion.lower() in ["sad", "depressed", "down", "hopeless"]:
            activity_benefit = (
                "gently activate your energy and connect with positive experiences"
            )
        elif emotion.lower() in ["angry", "frustrated", "irritated"]:
            activity_benefit = (
                "release tension and create space between feelings and reactions"
            )
        else:
    ...
    if state.get("reasoning_visible", True):
        ...
        new_messages.append(
            FunctionMessage(name="self_care_reasoning", content=reasoning_message)
        )
...
def generate_therapeutic_story(struggle: str, context: str = "") -> Dict[str, str]:
    ...
    story_prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=f"""You are a therapeutic storyteller who creates healing narratives based on narrative therapy principles.
        ...
        """
            ...
            HumanMessage(
                content=f"Create a therapeutic story for someone struggling with {struggle}. Additional context about their situation: {context}"
            ),
        ]
    )
    ...
    questions_section = re.search(
        r"##\s*Reflection Questions:?([\s\S]*?)($|#)", content + "#"
    )
...
def analyze_emotion_and_needs(state: AgentState) -> Dict:
    ...
    latest_user_msg = [msg for msg in messages if isinstance(msg, HumanMessage)][
        -1
    ].content
    ...
    emotion_analysis_prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=f"""You are a mental health professional who analyzes emotional content.
    ...
    combined_profile = {
        ...
        "facial_emotion": (
            facial_emotion.get("emotion", None) if facial_emotion else None
        ),
    ...
    if facial_emotion and voice_emotion:
        ...
        if has_positive and has_negative:
            ...
            combined_profile["conflict_note"] = (
                f"Detected conflicting signals: text suggests {text}, face suggests {face}, voice suggests {voice}"
            )
    ...
    if parsed_response.get("needs_immediate_resources", False) or parsed_response.get(
        "crisis_level"
    ) in ["high", "very_high"]:
    ...
    if state.get("reasoning_visible", True):
        ...
        state["messages"].append(
            FunctionMessage(name="thinking_process", content=reasoning_message)
        )
    ...
    return {
        ...
        "immediate_resources_needed": parsed_response.get(
            "needs_immediate_resources", False
        )
        or parsed_response.get("crisis_level") in ["high", "very_high"],
...
def select_and_use_tools(state: AgentState) -> Dict:
    ...
    if (
        query_route == "video_resources"
        or "youtube_videos" in response_strategy.get("appropriate_tools", [])
        or "video" in latest_user_msg.lower()
    ):
        try:
            ...
            if videos and "youtube.com/watch" in videos:
                ...
                if (
                    "blog" in latest_user_msg.lower()
                    or "summarize" in latest_user_msg.lower()
                    or user_preferences.get("prefers_detailed_content", False)
                ):
                    ...
                    if state.get("reasoning_visible", True):
                        ...
                        new_messages.append(
                            FunctionMessage(
                                name="blog_creation", content=blog_creation_message
                            )
                        )
                else:
    ...
    if query_route == "knowledge_base" or any(
        tool in response_strategy.get("appropriate_tools", [])
        for tool in ["mental_health_info", "knowledge"]
    ):
        # Prioritize our knowledge base
        tool_results["mental_health_info"] = search_mental_health_info(latest_user_msg)

    if query_route == "academic_research" or any(
        tool in response_strategy.get("appropriate_tools", [])
        for tool in ["research", "arxiv", "academic"]
    ):
    ...
    if state.get("reasoning_visible", True) and tool_results:
        ...
        new_messages.append(
            FunctionMessage(name="information_gathering", content=tools_summary)
        )
...
def generate_response(state: AgentState) -> Dict:
    ...
    if tool_results:
        ...
        for tool_name, result in tool_results.items():
            if tool_name == "youtube_content" and isinstance(result, dict):
                ...
                response_context += (
                    f"Key points: {', '.join(result.get('key_points', [])[:3])}\n"
                )
                response_context += (
                    f"Blog content: {result.get('content', '')[:300]}...\n"
                )
            elif tool_name == "youtube_videos":
    ...
    response_prompt = ChatPromptTemplate.from_messages(
        ...
        """
            ...
            HumanMessage(
                content="Provide a brief, helpful response. Be concise and include references."
            ),
        ]
    )
    ...
    if state.get("reasoning_visible", True):
        ...
        # Append directly to state messages
        state["messages"].append(
            FunctionMessage(name="thinking_process", content=final_reasoning)
        )
    ...
    if "video_blog" in tool_results and isinstance(tool_results["video_blog"], dict):
        ...
        if "title" in blog and "content" in blog:
            ...
            if "blog" not in content.lower():
                ...
                video_url = ""
                if (
                    "youtube_videos" in tool_results
                    and "youtube.com/watch" in tool_results["youtube_videos"]
                ):
    ...
    if "wikipedia" in tool_results:
        references.append(
            f"Source: Wikipedia article on {tool_results['wikipedia'].split(' - ')[0] if ' - ' in tool_results['wikipedia'] else 'the topic'}"
        )
...
def update_user_preferences(state: AgentState) -> Dict:
    ...
    if (
        "video" in latest_user_msg
        or "youtube" in latest_user_msg
        or "watch" in latest_user_msg
    ):
        preferences["likes_videos"] = True

    if (
        "brief" in latest_user_msg
        or "short" in latest_user_msg
        or "quick" in latest_user_msg
    ):
        preferences["prefers_brief_responses"] = True

    if (
        "resource" in latest_user_msg
        or "article" in latest_user_msg
        or "read" in latest_user_msg
    ):
        preferences["wants_resources"] = True

    if (
        "stop showing reasoning" in latest_user_msg
        or "hide reasoning" in latest_user_msg
    ):
    ...
    if (
        "blog" in latest_user_msg
        or "article" in latest_user_msg
        or "write-up" in latest_user_msg
    ):
...
def explain_tool_selection(state: AgentState) -> Dict:
    ...
    if appropriate_tools:
        ...
        # Append directly to state messages
        state["messages"].append(
            FunctionMessage(name="thinking_process", content=tools_reasoning)
        )
...
def track_mood(state: AgentState) -> Dict:
    ...
    if len(mood_history) >= 3 and is_mood_checkin:
        ...
        if most_common[1] >= 2:  # If an emotion appears at least twice
            ...
            new_messages.append(
                FunctionMessage(name="mood_tracking", content=insight_message)
            )

    return {"mood_history": mood_history, "messages": new_messages}
...
def process_mood_tracking(state: AgentState) -> Dict:
    ...
    asking_about_patterns = any(
        x in latest_user_msg.lower()
        for x in ["pattern", "history", "trends", "tracking", "journal"]
    )
    ...
    if asking_about_patterns and len(mood_history) > 3:
        ...
        insight_prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessage(
                    content="""You are analyzing mood tracking data to provide helpful insights.
            Format your response as bullet points with 2-3 observations and a gentle question to explore further."""
                ),
                HumanMessage(
                    content=f"Mood history (most recent first): {str(mood_history[:10])}"
                ),
            ]
        )
...
def provide_cbt_exercise(state: AgentState) -> Dict:
    ...
    messages = state["messages"]
    emotion_analysis = state.get("emotion_analysis", {})
    cbt_progress = state.get(
        "cbt_progress", {"exercises_completed": [], "current_focus": None}
    )
    ...
    if state.get("reasoning_visible", True):
        ...
        new_messages.append(
            FunctionMessage(name="cbt_reasoning", content=reasoning_message)
        )
...
def provide_education(state: AgentState) -> Dict:
    ...
    topic_prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content="Extract the main mental health topic from this query. Respond with just the topic name."
            ),
    ...
    if state.get("reasoning_visible", True):
        ...
        new_messages.append(
            FunctionMessage(name="education_reasoning", content=reasoning_message)
        )
...
def provide_therapeutic_story(state: AgentState) -> Dict:
    ...
    struggle_prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content="""Extract the main emotional struggle or challenge from this message.
    ...
    if state.get("reasoning_visible", True):
        ...
        new_messages.append(
            FunctionMessage(name="story_reasoning", content=reasoning_message)
        )
    ...
    return {"messages": new_messages + [AIMessage(content=story_message)]}
...
class QueryRouter(BaseModel):
    ...
    route_to: Literal[
        ...
        "resource_sharing",
    ] = Field(..., description="Route the query to the most appropriate resource.")
...
class ContentClassifier(BaseModel):
    ...
    content_type: Literal[
        ...
        "greeting",
        "gratitude",
    ] = Field(..., description="The primary type of content in the user's message.")
...
router_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """You are an expert at routing mental health queries to the most appropriate resources.
...
classifier_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """You are an expert at classifying mental health messages to help provide appropriate responses.
...
def provide_reflective_listening(state: AgentState) -> Dict:
    ...
    reflection_prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content="""You are a mental health assistant skilled in reflective listening.
    ...
    if state.get("reasoning_visible", True):
        ...
        new_messages.append(
            FunctionMessage(name="reflection_reasoning", content=reasoning_message)
        )
...
def provide_motivational_response(state: AgentState) -> Dict:
    ...
    motivation_prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content="""You are a compassionate mental health assistant skilled in motivation.
    ...
    if state.get("reasoning_visible", True):
        ...
        new_messages.append(
            FunctionMessage(name="motivation_reasoning", content=reasoning_message)
        )
...
def provide_resource_sharing(state: AgentState) -> Dict:
    ...
    if "depress" in primary_emotion.lower() or "sad" in primary_emotion.lower():
        ...
        "anx" in primary_emotion.lower()
        or "worry" in primary_emotion.lower()
        or "fear" in primary_emotion.lower()
    ):
    ...
    if state.get("reasoning_visible", True):
        ...
        new_messages.append(
            FunctionMessage(name="resource_reasoning", content=reasoning_message)
        )
...
workflow.add_conditional_edges(
    "crisis_resources",
    lambda state: (
        "mood_tracking" if state.get("query_route") == "mood_tracking" else "continue"
    ),
    {
...
def chat_with_mental_health_assistant(
    user_input: str, state: Optional[Dict] = None
) -> Dict:
    ...
    face_emotion_match = re.search(
        r"\[Detected facial expression: ([^\]]+)\]", user_input
    )
    if face_emotion_match:
        ...
        # Add to state
        state["facial_emotion"] = {"emotion": emotion_text, "score": confidence}
    ...
    if voice_emotion_match:
        ...
        # Add to state
        state["voice_emotion"] = {"emotion": emotion_text, "score": confidence}
