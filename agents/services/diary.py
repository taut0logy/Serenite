...
if not ASTRA_DB_APPLICATION_TOKEN or not ASTRA_DB_ID:
    logger.info("AstraDB credentials not found in environment variables")
...
try:
    ...
    logger.info("Successfully initialized AstraDB vector store")
except Exception as e:
    logger.info(f"Failed to initialize vector store: {str(e)}")
    raise
...
def analyze_mood(state: AgentState) -> AgentState:
    ...
    try:
        ...
        logger.info(f"Error analyzing mood: {str(e)}")
        return {
            "messages": messages,
            "mood": "neutral",
...
def analyze_diary_entry(entry: DiaryEntry) -> MoodAnalysis:
    ...
    try:
        ...
        logger.info(f"Error in mood analysis: {str(e)}")
        raise
...
def store_diary_entry(entry: DiaryEntry) -> StoredDiaryEntry:
    ...
    try:
        ...
        astra_vector_store.add_texts(texts=[entry.content], metadatas=[document])
        ...
        logger.info(f"Error storing diary entry: {str(e)}")
        raise
...
def search_diary_entries(
    ...
    try:
        ...
        logger.info(f"Error searching diary entries: {str(e)}")
        raise
