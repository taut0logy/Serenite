...
class KYCService:
    def __init__(self):
        ...
        logger.info(f"Enhanced KYC Service initialized with upload directory: {self.upload_dir}")
    ...
    def detect_and_extract_faces(self, image: np.ndarray, is_id_card: bool = False) -> List[Tuple[np.ndarray, Dict]]:
        """
        ...
        try:
            ...
            logger.info(f"Detected {len(faces)} faces in {'ID card' if is_id_card else 'selfie'}")
            ...
            for i, (x, y, w, h) in enumerate(faces):
                ...
                metadata = {
                    ...
                    'quality_score': self._calculate_face_quality(blur_score, brightness, face_area),
                    'is_id_card': is_id_card
                }
                
                faces_data.append((face_region, metadata))
                
                logger.info(f"Face {i}: Area={face_area}, Quality={metadata['quality_score']:.2f}, Blur={blur_score:.2f}")
    ...
    def _calculate_face_quality(self, blur_score: float, brightness: float, face_area: int) -> float:
        ...
        try:
            ...
            brightness_quality = max(0, 100 - (brightness_diff / optimal_brightness) * 100)
            
            # Size quality (larger faces are generally better)
            size_quality = min(100, face_area / 5000 * 100)  # Normalize based on typical face size
            
            # Weighted average
            quality_score = (blur_quality * 0.4 + brightness_quality * 0.3 + size_quality * 0.3)
    ...
    def select_best_face(self, faces_data: List[Tuple[np.ndarray, Dict]]) -> Tuple[np.ndarray, Dict]:
        ...
        logger.info(f"Selected best face with quality score: {best_metadata['quality_score']:.2f}")
    ...
    async def save_image(self, file_data: bytes, user_id: str, image_type: str) -> str:
        ...
        try:
            ...
            if img is None:
                ...
                raise HTTPException(status_code=400, detail="Invalid image data. Please upload a valid image file.")
            
            # Validate image dimensions
            height, width = img.shape[:2]
            if width < 100 or height < 100:
                raise HTTPException(status_code=400, detail="Image resolution too low. Minimum 100x100 pixels required.")
    ...
    async def verify_identity(self, selfie_path: str, id_card_path: str) -> Dict[str, Any]:
        """
        ...
        """
        try:
            logger.info(f"Starting identity verification - Selfie: {selfie_path}, ID: {id_card_path}")
            ...
            if selfie_img is None:
                raise HTTPException(status_code=404, detail="Selfie image not found or corrupted")
            if id_card_img is None:
                raise HTTPException(status_code=404, detail="ID card image not found or corrupted")
            
            # Step 1: Extract faces from selfie
            selfie_faces = self.detect_and_extract_faces(selfie_img, is_id_card=False)
            if not selfie_faces:
                raise HTTPException(status_code=400, detail="No face detected in selfie. Please ensure your face is clearly visible.")
            
            # Step 2: Extract faces from ID card
            id_faces = self.detect_and_extract_faces(id_card_img, is_id_card=True)
            if not id_faces:
                raise HTTPException(status_code=400, detail="No face detected in ID card. Please ensure the ID photo is clear and visible.")
            ...
            cv2.imwrite(selfie_face_path, cv2.cvtColor(best_selfie_face, cv2.COLOR_RGB2BGR))
            ...
            for model in models:
                try:
                    ...
                    logger.info(f"{model} verification: {result['verified']} (distance: {result['distance']:.4f})")
                except Exception as e:
                    logger.warning(f"Verification with {model} failed: {str(e)}")
                    continue
            
            if not verification_results:
                raise HTTPException(status_code=500, detail="All face verification models failed")
            ...
            quality_factor = min(selfie_metadata['quality_score'], id_metadata['quality_score']) / 100
            ...
            result = {
                ...
                'message': self._get_verification_message(is_verified, final_confidence * 100)
            }
            
            # Cleanup extracted face images
            self._safe_delete([selfie_face_path, id_face_path])
            
            logger.info(f"Identity verification completed: {result['verified']} with {result['confidence_score']}% confidence")
