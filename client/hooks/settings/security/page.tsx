...
import { AlertCircle, CheckCircle, Shield, LockKeyhole, ArrowLeft, RefreshCw } from 'lucide-react';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { ScrollArea } from '@/components/ui/scroll-area';
import Link from 'next/link';
import { ENABLE_TWO_FACTOR, DISABLE_TWO_FACTOR, REGENERATE_BACKUP_CODES, REMOVE_TRUSTED_DEVICE, GET_TWO_FACTOR_STATUS, GET_BACKUP_CODES, GET_TRUSTED_DEVICES } from '@/graphql/operations';
...
export default function SecurityPage() {
  ...
  const [disable2FA, { loading: disabling }] = useMutation(DISABLE_TWO_FACTOR);
  const [regenerateCodes, { loading: regenerating }] = useMutation(REGENERATE_BACKUP_CODES);
  
  const { data: statusData, loading: loadingStatus, refetch: refetchStatus } = useQuery(
  ...
  const { data: backupCodesData, loading: loadingBackupCodes, refetch: refetchBackupCodes } = useQuery(
    GET_BACKUP_CODES,
    {
      variables: { userId },
  ...
  const { data: devicesData, loading: loadingDevices, refetch: refetchDevices } = useQuery(
    GET_TRUSTED_DEVICES,
    {
      variables: { userId },
      skip: !userId,
  ...
  useEffect(() => {
    ...
    if (statusData?.getUserTwoFactorStatus?.success) {
      setIs2FAEnabled(statusData.getUserTwoFactorStatus.twoFactorEnabled || false);
    }
  }, [statusData]);
          ...
          <CardContent>
            <div className="flex items-center justify-between py-4">
              ...
              <Switch
                ...
                disabled={enabling || disabling || loadingStatus}
              />
            </div>
          ...
          <CardContent>
            {loadingDevices ? (
                ...
                {devices.map((device) => (
                  <div key={device.id} className="flex items-center justify-between p-4 border rounded-md">
                    ...
                    <Button 
                      ...
                      disabled={removingDevice === device.deviceToken}
                    >
                      {removingDevice === device.deviceToken ? 'Removing...' : 'Remove'}
                    </Button>
                  </div>
                ))}
              </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  You have no trusted devices. When you log in, check `&ldquo;Trust this device&rdquo;` to add one.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
                  ...
                  backupCodes.map((code, index) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-muted-foreground">
                        {index + 1} of {backupCodes.length}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="text-center text-muted-foreground py-8">
                    No backup codes available. Click `&ldquo;Regenerate Codes&rdquo;` to create new backup codes.
                  </p>
                )}
              </div>
            </ScrollArea>
