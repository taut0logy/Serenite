...
interface VoiceEmotionCaptureProps {
    isOpen: boolean;
    onClose: () => void;
...
const VoiceEmotionCapture: React.FC<VoiceEmotionCaptureProps> = ({ isOpen, onClose, onEmotionDetected }) => {
    ...
    const startRecording = async () => {
        try {
            ...
            setError(
                "Could not access microphone. Please check permissions or try the upload option."
            );
        }
    };
    ...
    const analyzeEmotion = async () => {
        ...
        try {
            ...
            const data = response.data; // Fixed: use response.data instead of response.json()
            setDetectedEmotion(data);
        } catch (err) {
            console.error("Error analyzing voice emotion:", err);
            setError(
                "Failed to detect emotion. Please try again or use a clearer recording."
            );
        } finally {
            setIsLoading(false);
        }
                            ...
                            {!audioUrl ? (
                                <>
                                    ...
                                    <input
                                        ...
                                        title="Upload audio file for emotion analysis"
                                    />
                                </>
                            ) : (
                                <>
