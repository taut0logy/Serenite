...
import { Users, Calendar, MessageCircle, Video, Mic, Settings, Bell, Lock, Shield } from "lucide-react";
...
const groups: SupportGroup[] = [
  {
    ...
    description: "A nurturing space for practicing mindfulness and supporting mental well-being",
    privacy: "public",
    supportType: "video",
    owner: {
      name: "Sarah Chen",
...
export default function MyGroups() {
  ...
  const getActivityColor = (level: number) => {
    if (level >= 80) return "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300";
    if (level >= 50) return "bg-yellow-100 text-yellow-700 dark:bg-yellow-500/20 dark:text-yellow-300";
        ...
        <header className="mb-8">
          ...
          <div className="flex items-center justify-between mt-6">
            ...
            <div className="flex gap-4">
              <Button variant="outline" onClick={() => setViewType(viewType === 'grid' ? 'list' : 'grid')}>
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h3 className="text-xl font-semibold font-['Space_Grotesk']">{group.name}</h3>
                              {group.privacy === 'private' && <Lock className="h-4 w-4 text-purple-500" />}
                            </div>
                            <div className="flex flex-wrap gap-2 mt-2">
                              {group.focusTags.map((tag) => (
                                <Badge key={tag} variant="secondary" className="bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300">
                                  {tag}
                                </Badge>
                              ))}
                            </div>
                          ...
                          <Badge 
                            variant="outline" 
                            className={`ml-auto ${getActivityColor(group.activityLevel)}`}
                          >
                            {group.activityLevel}% Active
                          </Badge>
                        </div>
                        ...
                        <div className="mt-4">
                          <div className="flex items-center gap-6 text-sm text-muted-foreground">
                            ...
                            {group.supportType === 'video' && <Video className="h-4 w-4" />}
                            {group.supportType === 'voice' && <Mic className="h-4 w-4" />}
                            <Shield className="h-4 w-4 text-green-500" title="Actively Moderated" />
                          </div>
                        </div>
                        ...
                        <div className="mt-4">
                          ...
                          <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                            <motion.div 
                              ...
                              initial={{ width: 0 }}
                              animate={{ width: `${group.helpedPercentage}%` }}
                  ...
                  <HoverCardContent className="w-80 p-0 backdrop-blur-lg bg-white/95 dark:bg-gray-900/95 border-none shadow-xl">
                    <div className="p-6">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          <p className="font-semibold">Led by {group.owner.name}</p>
                          <p className="text-sm text-muted-foreground">
                            {group.owner.reputation}% positive feedback
                          </p>
                        </div>
                      </div>
                      ...
                      {group.nextMeeting && (
                        <div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-lg mb-4">
                          <div className="flex items-center gap-2 text-blue-700 dark:text-blue-300">
                            ...
                            <span className="text-sm font-medium">Next Meeting: {group.nextMeeting}</span>
                          </div>
                        </div>
                      )}
